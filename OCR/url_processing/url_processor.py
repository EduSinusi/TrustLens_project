...
def check_url_in_firestore(url: str) -> tuple[dict, bool]:
    ...
    try:
        ...
        if query:
            ...
            required_apis = {"virustotal", "whois"}
...
def evaluate_url(url: str) -> dict:
    """Evaluate URL using VirusTotal and WHOIS."""
    print(f"Evaluating URL: {url}")
    # Get simplified results for WHOIS
    ...
    results = {
        "virustotal": virustotal_full_result,  # Store the full result
        "whois": whois_result
    }
    ...
    if any(status == "Unsafe" for status in statuses):
        overall = "Unsafe"
    elif all(status == "Safe" for status in statuses):
        overall = "Safe"
    else:
        overall = "Unknown"

    safety_status = {"overall": overall, "details": results}
