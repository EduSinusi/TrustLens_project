...
const UnsafeScanSection = ({ scans, selectedScan, setSelectedScan, isVirusTotalPopupOpen, setIsVirusTotalPopupOpen, isSecurityPopupOpen, setIsSecurityPopupOpen }) => {
  ...
  return (
    <div className="mb-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Unsafe Scan Results</h2>
      <div className="overflow-x-auto hover:bg-gray-50 transition-colors">
        <table className="w-full text-left text-md text-gray-600">
          <thead>
            <tr className="text-lg bg-red-600 text-white font-semibold">
              <th className="py-3 px-4">Domain</th>
              <th className="py-3 px-4">Status</th>
              <th className="py-3 px-4">Timestamp</th>
              <th className="py-3 px-4">Block Status</th>
          ...
          <tbody>
            {scans.map((scan) => (
              <tr key={scan.id} className="border-t border-gray-200 hover:bg-gray-100 transition-colors">
                ...
                <td className="py-3 px-4 text-red-600 font-medium">Unsafe</td>
                <td className="py-3 px-4">{scan.timestamp.toLocaleString()}</td>
                <td className="py-3 px-4">
                  {scan.safety_status.details?.url_info?.block_status === null
                    ? "Not Blocked"
                    ...
                    ? "Blocked"
                    : scan.safety_status.details?.url_info?.block_status || "N/A"}
                </td>
                <td className="py-3 px-4">
...
export default UnsafeScanSection;