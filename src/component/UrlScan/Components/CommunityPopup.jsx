...
const CommunityPopup = ({ isOpen, onClose, url, userId }) => {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      const totalRatings = urlComments.reduce((sum, c) => sum + (c.rating || 0), 0);
      const avgRating = urlComments.length > 0 ? (totalRatings / urlComments.length).toFixed(1) : "0.0";
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900/60 z-50 transition-opacity duration-300">
      <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl w-full max-w-5xl h-[70vh] flex transform transition-all duration-300 scale-100 hover:scale-[1.005] border border-indigo-200/30">
        ...
        <div className="w-1/2 p-5 overflow-y-auto rounded-tr-3xl " ref={chatContainerRef} style={{ scrollbarWidth: "thin", scrollbarColor: "#9CA3AF #F3F4F6" }}>
          <div className="bg-gradient-to-r from-indigo-600 to-purple-500 text-white text-center py-4 rounded-tl-3xl rounded-tr-3xl animate-pulse-once">
          ...
          {loading ? (
            ...
            <div className="space-y-4 mt-4">
              ...
              <div className="bg-gradient-to-r from-indigo-100 to-purple-100 p-3 rounded-2xl shadow-md">
                <h4 className="text-lg font-semibold text-gray-800">Average Rating</h4>
                <p className="text-xl font-bold text-indigo-700">
                  {comments.reduce((sum, c) => sum + (c.rating || 0), 0) / (comments.length || 1).toFixed(1)}/5
                </p>
              </div>
              {comments.map((c) => (
